    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            ...
            const assistantMessage = {
                ...
                timestamp: new Date(),
                releventChunks: response.data.releventChunks
            };
    ...
    const renderMessage = (msg, index) => {
        ...
        return (
            <div key={index} className={`flex items-start gap-3 my-4 ${isUser ? 'justify-end' : ''}`}>
                {
                    !isUser && (
                        <div className="w-9 h-9 rounded-xl bg-linear-to-br from-emerald-400 to-teal-500 shadow-lg shadow-emerald-500/50 flex items-center justify-center shrink-0">
                            <Sparkles className='w-4 h-4 text-white' strokeWidth={2} />
                        </div>
                    )
                }
                <div className={`max-w-lg p-4 ronded-2xl shadow-sm ${
                    isUser
                ...
                {
                    isUser && (
                        <div className="w-9 h-9 rounded-xl bg-linear-to-br from-slate-200 to-teal-300 flex items-center justify-center text-slate-700 font-semibold text-sm shrink-0 shadow-sm ">
        ...
        <div className="flex flex-col h-[70vh] bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 overflow-hidden">
                ...
                <div ref={messagesEndRef} />
                {
                    loading && (
                        <div className="flex items-center gap-3 my-4">
                            ...
                            <div className="flex items-center gap-2 px-4 py-3 rounded-2xl rounded-bl-md bg-white border border-slate-600">
            ...
            <div className="p-5 border-t border-slate-200/60 bg-white/80">
                <form onSubmit={handleSendMessage} className="flex items-center gap-3">
                    ...
                    />
                    <button
                        className="shrink-0 w-12 h-12 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white rounded-lg flex items-center justify-center transition-all duration-200 shadow-lg shadow-emerald-500/25 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95"
                        type='submit'
