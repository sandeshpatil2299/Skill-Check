...
export const chunkText = (text, chunkSize = 500, overlap = 50) => {
    ...
    for (const paragraph of paragraphs) {
        ...
        //If addint this paragrah exceeds chunk size, save current chunk
...
/**
 * Find relevan chunks based on keywords matching
...
export const findRelevantChunks = (chunks, query, maxChunks = 3) => {
    ...
    const stopWords = new Set([
        'the', 'is', 'at', 'which', 'on', 'a', 'and', 'or', 'but', 'in', 'with', 'to', 'for', 'of', 'as', 'by', 'this', 'that', 'it'
    ]);
    ...
    const scoredChunks = chunks.map((chunk, index) => {
        ...
        for (const word of queryWords) {
            ...
            const partialMatchs = (content.match(new RegExp(word, 'g')) || []).length;
            score += Math.max(0, partialMatchs - exactMatches) * 1.5;
        }
